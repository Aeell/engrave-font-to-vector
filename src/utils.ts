...
export function mmToFontSize(mm: number): number {
  return mm * 72 / 25.4;
}

/** Round to 0.001 mm by default */
export function roundMm(x: number, decimals = 3): number {
  const f = Math.pow(10, decimals);
  return Math.round(x * f) / f;
}

/** Flatten an SVG path into polylines with given chord tolerance (mm). */
export function flattenSvgPath(d: string, toleranceMm: number): number[][] {
